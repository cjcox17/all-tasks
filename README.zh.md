...
- **工作区总览列表**：打开看板首先看到的是类 Monday.com 的看板表格——列头（工作区 / 待办 / 待批准 / 进行中 / 定时 / 已完成 / 已失败 / 合计）下每个工作区一行，带彩色头像与右对齐的等宽数字计数，外加一行**全部任务**汇总所有工作区（含未分配任务）。点击该行进入其工作区的看板；▸ 箭头把该行就地展开为它的分组（彩色分组头，含名称胶囊、模式徽标、成员数与独立的分组折叠开关）与任务行（状态圆点、标题、对齐的状态标签与徽标），点击任务行打开任务详情；⚙ 按钮打开工作区默认设置。看板始终按工作区收窄，头部的**返回**按钮回到总览，原来的工作区下拉已移除。每列先显示该工作区钉住的任务，再显示**未分配**区（没有钉的任务，运行时回退到最近工作区），任何视图都不会丢失任务；主列只显示该工作区的分组，未分配范围的分组在**未分配**区内展示。从运行列表删除的工作区（在侧边栏移除）在工作区基线加载完成后会连同其任务与分组一起从看板消失：总览不再合成已消失工作区的行，其钉住的任务也会从总览计数、看板（各列与归档）和仪表盘中过滤掉。账本中的钉保持不变——任务仍被记录，只是不再显示。
