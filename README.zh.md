...
- **工作区默认设置**：每个工作区可以保存一组执行默认值（agent 预设、模型与推理强度、端点顺序、权限，以及"新任务默认未批准"开关），在**该工作区的看板**里新建任务时自动预填——总览卡片或看板头部都提供 ⚙ 入口。默认值由 Host 账本持久化（键为工作区 id，记录非空即存），留空的字段表示"使用运行时默认"，与未钉任务一致；把全部字段清空会删除该工作区的默认记录。
...
默认值由 Host 账本持久化：`workspaceDefaults` 映射以工作区 id 为键，记录非空即存；通过协议 `set-workspace-defaults` 动作编辑（补丁语义：`null` 清除字段，全部字段清空即删除该工作区的记录）。入口在工作区总览卡片与看板头部的 ⚙ 按钮；留空的字段表示"使用运行时默认"，与未钉任务完全一致。默认值只是新建时的预填——已有任务不受影响，也绝不参与运行时的 fail-closed 检查。
...
7. 在工作区总览的卡片（或看板头部的 ⚙）打开**工作区默认设置**：设置模式、模型、端点与权限并勾选「新任务默认未批准」，保存；在该工作区的看板新建任务，确认弹窗已选中该工作区并预填这些默认值、未批准开关已勾选，创建出的任务带**未批准**标记且无法运行。把默认值全部清空保存，确认该工作区的默认记录被删除（新建任务不再预填）。
