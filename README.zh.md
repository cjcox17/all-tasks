...
- **工作区总览网格**：打开看板首先看到的是工作区总览——每个工作区一张卡片，显示实时的任务计数（待办 / 待批准 / 进行中 / 定时 / 已完成 / 已失败），外加一张**全部任务**卡片汇总所有工作区（含未分配任务）。点击卡片进入该工作区的看板；看板始终按工作区收窄，头部的**返回**按钮回到总览，原来的工作区下拉已移除。每列先显示该工作区钉住的任务，再显示**未分配**区（没有钉的任务，运行时回退到最近工作区），任何视图都不会丢失任务；任务分组保持完整，显示所选工作区的成员加未钉成员。被任务钉住但从运行列表消失的工作区（已删除或重命名）仍会出现在总览里，避免钉住的任务无处可寻。
- **工作区默认设置**：每个工作区可以保存一组执行默认值（agent 预设、模型与推理强度、端点顺序、权限，以及"新任务默认未批准"开关），在**该工作区的看板**里新建任务时自动预填——总览卡片或看板头部都提供 ⚙ 入口。默认值由 Host 账本持久化（键为工作区 id，记录非空即存）。任务自身的执行目标在运行时优先；任务留空的任何目标都会回退到其工作区的默认值，最后才落到部署默认——因此在默认值设置前创建、或字段留空的任务，运行时也会使用工作区的设置。
...
默认值由 Host 账本持久化：`workspaceDefaults` 映射以工作区 id 为键，记录非空即存；通过协议 `set-workspace-defaults` 动作编辑（补丁语义：`null` 清除字段，全部字段清空即删除该工作区的记录）。入口在工作区总览卡片与看板头部的 ⚙ 按钮。
...
6. 打开看板，确认首先显示**工作区总览网格**：每张卡片有实时计数，外加**全部任务**卡片。把一个任务钉到一个工作区，另一个任务不钉；点击该工作区卡片进入其看板，确认列中显示钉住的任务外加**未分配**区（含未钉任务），钉到其他工作区的任务消失；点**返回**回到总览，再点**全部任务**卡片确认未收窄总览恢复。
7. 在工作区总览的卡片（或看板头部的 ⚙）打开**工作区默认设置**：设置模式、模型、端点与权限并勾选「新任务默认未批准」，保存；在该工作区的看板新建任务，确认弹窗已选中该工作区并预填这些默认值、未批准开关已勾选，创建出的任务带**未批准**标记且无法运行。再在该工作区新建任务并清空其模式/模型/端点/权限字段：确认任务详情在这些留空字段旁显示「工作区默认：…」提示，执行该任务时使用的是工作区的预设、模型、端点和权限（执行记录里能看到默认端点）。把默认值全部清空保存，确认该工作区的默认记录被删除（新建任务不再预填，留空字段回退到部署默认）。
