...
- **工作区总览列表**：打开看板首先看到的是类 Monday.com 的总览目录——每个工作区一行，带彩色头像与稀疏计数徽标（只显示有任务的状态：待办 / 待批准 / 进行中 / 定时 / 已完成 / 已失败），外加一行**全部任务**汇总所有工作区（含未分配任务）。点击该行进入其工作区的看板；▸ 箭头把该行就地展开为它的分组（彩色分组带，含模式徽标与成员数）与任务行，点击任务行打开任务详情；⚙ 按钮打开工作区默认设置。看板始终按工作区收窄，头部的**返回**按钮回到总览，原来的工作区下拉已移除。每列先显示该工作区钉住的任务，再显示**未分配**区（没有钉的任务，运行时回退到最近工作区），任何视图都不会丢失任务；任务分组保持完整，显示所选工作区的成员加未钉成员。被任务钉住但从运行列表消失的工作区（已删除或重命名）仍会出现在总览里，避免钉住的任务无处可寻。
...
打开看板首先看到**工作区总览列表**：每个工作区一行（实时计数：待办 / 待批准 / 进行中 / 定时 / 已完成 / 已失败），外加一行**全部任务**汇总所有工作区（含未分配任务）。每行带彩色头像与稀疏计数徽标（只显示有任务的状态）；▸ 箭头把该行就地展开为它的分组（彩色分组带，含模式徽标与成员数）与任务行，点击任务行打开任务详情。点击该行进入其工作区的看板——看板始终按工作区收窄（任务的 `workspaceId` 钉——会话运行所在的工作区）；**全部任务**行打开未收窄的总览。看板头部的**返回**按钮回到总览列表，原来的**工作区**下拉已移除。收窄视图下每列先显示该工作区钉住的任务，再显示**未分配**区（没有钉的任务，运行时回退到最近工作区），任何视图都不会丢失任务。任务分组保持完整：分组区保留所选工作区的成员及其未钉成员，钉到其他工作区的成员从该视图消失。该筛选同样作用于归档视图，并与文本搜索叠加。它只是视图状态——不涉及任务、账本或协议的变更。被任务钉住但从运行列表消失的工作区（已删除或重命名）仍会出现在总览里，避免钉住的任务无处可寻。
...
6. 打开看板，确认首先显示**工作区总览列表**：每行有实时计数，外加**全部任务**行。点击某工作区行的 ▸ 箭头，确认其分组与任务行就地展开，点击任务行可打开任务详情。把一个任务钉到一个工作区，另一个任务不钉；点击该工作区行进入其看板，确认列中显示钉住的任务外加**未分配**区（含未钉任务），钉到其他工作区的任务消失；点**返回**回到总览，再点**全部任务**行确认未收窄总览恢复。
