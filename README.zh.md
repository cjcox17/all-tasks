...
- **默认**：手动创建（新建任务弹窗）的任务默认已批准。通过协议 `create` 动作的程序化创建（脚本、其他工具或 agent）**默认创建为未批准**——适合任何需要人工签字后才能运行的来源（例如其他工具为当天队列创建的任务）。确实需要立即执行的程序化创建者会显式传入 `approved: true`（事件 Webhook 的 autoRun 流程正是如此）；看板弹窗始终声明 `user` 来源。
...
- **来源**：每个任务都会记录创建者——看板弹窗（`user`）、通过协议 `create` 动作的脚本/工具（`api`）或事件 Webhook（`event`）。非 `user` 来源的任务在卡片、工作区列表行和详情视图上带**API** / **Webhook** 徽标，让你工作时「自己冒出来」的任务一眼就能和亲手创建的任务区分开；而且这些任务默认未批准，未经你批准绝不会出现并自行启动。
