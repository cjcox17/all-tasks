...
- **工作区总览列表**：打开看板首先看到的是类 Monday.com 的看板表格——列头（工作区 / 待办 / 待批准 / 进行中 / 定时 / 已完成 / 已失败 / 合计）下每个工作区一行，带彩色头像与右对齐的等宽数字计数，外加一行**全部任务**汇总所有工作区（含未分配任务）。点击该行进入其工作区的看板；▸ 箭头把该行就地展开为它的分组（彩色分组头，含名称胶囊、模式徽标、成员数与独立的分组折叠开关）与任务行（状态圆点、标题、对齐的状态标签与徽标），点击任务行打开任务详情；⚙ 按钮打开工作区默认设置。看板始终按工作区收窄，头部的**返回**按钮回到总览，原来的工作区下拉已移除。每列先显示该工作区钉住的任务，再显示**未分配**区（没有钉的任务，运行时回退到最近工作区），任何视图都不会丢失任务；任务分组保持完整，显示所选工作区的成员加未钉成员。被任务钉住但从运行列表消失的工作区（已删除或重命名）仍会出现在总览里，避免钉住的任务无处可寻。
...
- **成员关系**：一个任务最多属于一个分组；在新建任务弹窗或任务详情（分组栏）中选择，或直接把卡片拖到分组横幅/区域内即可加入（追加到组内顺序末尾）。把成员拖到列背景（所有分组之外）即移出分组。加入分组会把成员追加到组内顺序；移出即取消分组。删除分组会取消所有成员的分组（任务保留），且当任一成员存在未结算运行时会拒绝删除。
