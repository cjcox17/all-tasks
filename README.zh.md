...
端点在 `task-board` 设置命名空间下。设置卡片（**全部任务 → 设置 → 端点**）是常规编辑器：列出每个端点、支持增删改排序并一次性保存整张列表——任务弹窗的「端点」下拉与路由器都会实时重载，无需重启。同样的值也以 YAML 形式存放在 `task-board` 下，可直接编辑（路由器同样实时重载）：
...
DSH 在模型请求 300 秒内未收到新内容（stream-idle watchdog 默认值）时中止它。本地 LLM 重新计算大消息链，或计算速度较慢的模型，即使后端仍在生成 token 也可能超过该窗口，导致执行持续失败。设置卡片（全部任务 → 设置）列出每个可配置的提供方——`llm-pi-ai` 下的每个自定义/本地路由（LM Studio、Ollama、任意 OpenAI 兼容网关）加官方 `deepseek-official` 路由——并显示其当前生效的空闲超时；pi-ai 提供方还可设置可选的总请求上限：
...
4. 打开设置卡片 → **端点**，添加一个端点（如服务于本地提供方的 `lm-studio`），确认新建任务弹窗的「端点」下拉立即列出它。再配置一个 `offPeakOnly: true`（或窄 `allowedHours`）的端点并钉到任务上，在时段外执行：任务显示**等待端点**，不创建会话，时段一开自动启动；同时确认执行记录里显示实际使用的端点。
...
7. 在设置卡片中打开**模型默认超时**，把一个本地提供方（如 `lm-studio`）的空闲超时提高到 600 秒并保存；确认 `~/.dsh/settings.yaml` 出现 `llm-pi-ai.providers.lm-studio.streamIdleTimeoutMs`，然后用该模型执行任务，确认生成块之间的长空闲间隙不再失败。**恢复默认** 会移除覆盖并回到 300 秒。
