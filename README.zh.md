...
- **工作区视图**：看板可按单个执行工作区（任务的 `workspaceId` 钉——会话运行所在的工作区）收窄，**全部工作区** 仍保留总览。收窄视图下每列先显示该工作区钉住的任务，再显示**未分配**区（没有钉的任务，运行时回退到最近工作区），任何视图都不会丢失任务；任务分组保持完整，显示所选工作区的成员加未钉成员。
...
分组是看板级实体，在界面中创建（头部 **+ 新建分组**），持久化在 Host 账本中，并在每个看板列内以横幅聚合展示成员卡片。空分组（没有任何成员）也会显示在「待办」列，创建后立即可见、可编辑、可删除：

- **成员关系**：一个任务最多属于一个分组；在新建任务弹窗或任务详情（分组栏）中选择。加入分组会把成员追加到组内顺序；移出即取消分组。删除分组会取消所有成员的分组（任务保留），且当任一成员存在未结算运行时会拒绝删除。
- **执行模式**：顺序模式同一时刻只运行一个成员，按组内成员顺序——成员结算后，下一个可运行成员（待规划/待办、未归档、已批准）自动启动。并行模式最多同时运行配置的 `maxParallel` 个（留空为不限）。组内任务的任何启动——手动、分组 cron、路由自动启动——都受分组的容量与窗口限制，未批准成员绝不会被启动（序列跳过它，继续下一个已批准成员）。
...
- **整组移动**：把分组横幅拖到「待规划」或「待办」列可一次移动全部成员（任一成员运行中或分组已停止时不允许），不必逐个拖动卡片。
...
看板头部的**工作区**下拉可把看板收窄到单个执行工作区（任务的 `workspaceId` 钉——会话运行所在的工作区）；**全部工作区** 是默认总览，行为不变。收窄视图下每列先显示该工作区钉住的任务，再显示**未分配**区（没有钉的任务，运行时回退到最近工作区），任何视图都不会丢失任务。任务分组保持完整：分组区保留所选工作区的成员及其未钉成员，钉到其他工作区的成员从该视图消失。该筛选同样作用于归档视图，并与文本搜索叠加。它只是视图状态——不涉及任务、账本或协议的变更。
...
6. 把一个任务钉到一个工作区，另一个任务不钉；在头部**工作区**下拉选中该工作区，确认列中显示钉住的任务外加**未分配**区（含未钉任务），钉到其他工作区的任务消失；切回**全部工作区**确认总览恢复。
