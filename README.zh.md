...
- **看板数据看板与工作区控制**：打开看板首先看到汇总条——任务（待办 / 待批准 / 进行中 / 排队）、已完成（含成功率）、失败（含执行次数）、定时与分组、token 用量与预估费用。token 用量在每次执行结算时从会话的 `assistant/message` 用量采集（输入/输出/缓存/推理），因此只有本改动之后启动的运行才带有用量；预估费用按设置的每 100 万 token 单价（设置 → 插件 → 全部任务）乘以这些 token 计算，两者都具备前显示「—」。工作区行默认展开（分组与工作区均可折叠），每行带 ▶ 运行 / ⏸ 暂停 / ⏹ 停止：运行启动其未分组的**待办**任务（不含待规划）与非停止的分组；暂停停止其运行中的分组（可恢复）；停止取消其运行中的未分组任务与分组。
...
在**执行时**，工作区默认值作为任务自身目标之下的回退层：任务自己的模式/模型/端点/权限设置优先；留空的字段回退到工作区默认值；两边都留空才使用部署默认。该规则覆盖所有启动路径——手动执行、重新执行、定时任务、排队运行与分组自动推进——并且工作区默认的模式/模型/权限与任务显式钉住一样经过 fail-closed 检查（默认预设损坏或默认模型选择被拒绝会在发送 Prompt 前失败）。`approved` 默认值刻意不参与运行时回退：它只作用于**创建**（新任务默认未批准），绝不影响已有任务自身的审批状态。任务详情的执行设置里，凡是任务留空而工作区提供了默认值的字段，都会显示「工作区默认：…」提示。
...
7. 在工作区总览的行（或看板头部的 ⚙）打开**工作区默认设置**：设置模式、模型、端点与权限并勾选「新任务默认未批准」，保存；在该工作区的看板新建任务，确认弹窗已选中该工作区并预填这些默认值、未批准开关已勾选，创建出的任务带**未批准**标记且无法运行。再在该工作区新建任务并清空其模式/模型/端点/权限字段：确认任务详情在这些留空字段旁显示「工作区默认：…」提示，执行该任务时使用的是工作区的预设、模型、端点和权限（执行记录里能看到默认端点）。把默认值全部清空保存，确认该工作区的默认记录被删除（新建任务不再预填，留空字段回退到部署默认）。
...
13. 创建一个并行分组，包含两个待办成员和一个额外成员；打开分组编辑器（⚙），把额外成员选为**最终步骤**并保存。确认最终步骤卡片显示**最终步骤**徽标且在其他成员未完成时没有运行按钮，横幅显示**最终步骤 · 等待中**。启动分组：只有两个并行成员启动，手动运行最终步骤会被拒绝（`final step waits for all group members to settle`）。把其中一个成员移到**已失败**并结算另一个：确认最终步骤自动启动（默认情况下失败成员也会打开闸门）。再编辑分组勾选**要求其他成员全部成功**，把成员移回待办并重跑（其中一个失败），确认最终步骤保持不动，直到该成员成功为止。
