...
- **端点模型路由器**：命名计算端点（DeepSeek Official、NAS 上的 LM Studio 等），每个端点有并发上限、token 上限、可选可用时段与 off-peak-only 开关。端点在设置卡片中创建与编辑（**设置 → 插件 → 全部任务 → 端点**）：整表编辑器把 `task-board` 命名空间实时写回，任务弹窗的「端点」下拉与路由器立即刷新。路由器按任务的端点优先级顺序选择第一个可用端点启动每次运行，端点缺少 token 空间或处于时段外时自动回落到下一个。所有端点都被阻塞时，运行进入排队（不创建会话、不产生费用、重启后仍等待），时段一开或并发空出就自动启动，超过可配置的等待上限（默认 24 小时）才失败。峰值/低谷时段默认采用 DeepSeek 官方 off-peak 窗口（16:30–00:30 UTC，chat 半价 / reasoner 2.5 折），支持全局与按端点覆盖。
...
- **模型默认超时**：设置卡片按提供方调整模型请求的默认超时。DSH 在请求 300 秒内未收到新内容（stream-idle watchdog 默认值）时中止它；本地 LLM 重新计算大消息链或生成较慢时，即使后端仍在产出 token 也会触发。每个已配置的提供方（自定义/本地 `llm-pi-ai` 路由加官方 DeepSeek 路由）都会显示其生效的空闲超时与可选的总请求上限；保存即写入该提供方自己的 DSH 设置，对该提供方的下一个请求立即生效，对话与任务执行都受影响。
...
端点在 `task-board` 设置命名空间下。设置卡片（**设置 → 插件 → 全部任务 → 端点**）是常规编辑器：列出每个端点、支持增删改排序并一次性保存整张列表——任务弹窗的「端点」下拉与路由器都会实时重载，无需重启。同样的值也以 YAML 形式存放在 `task-board` 下，可直接编辑（路由器同样实时重载）：
...
DSH 在模型请求 300 秒内未收到新内容（stream-idle watchdog 默认值）时中止它。本地 LLM 重新计算大消息链，或计算速度较慢的模型，即使后端仍在生成 token 也可能超过该窗口，导致执行持续失败。设置卡片（设置 → 插件 → 全部任务）列出每个可配置的提供方——`llm-pi-ai` 下的每个自定义/本地路由（LM Studio、Ollama、任意 OpenAI 兼容网关）加官方 `deepseek-official` 路由——并显示其当前生效的空闲超时；pi-ai 提供方还可设置可选的总请求上限：

- **空闲超时（秒）**：请求在此秒数内未收到任何新内容块即中止；默认 300。
- **总请求超时（秒）**：整个请求的最长时长（仅 pi-ai 提供方）；留空使用后端默认。

保存即写入该提供方自己的 DSH 设置——`streamIdleTimeoutMs`，pi-ai 路由还有 `timeoutMs`，例如 `~/.dsh/settings.yaml` 中的 `llm-pi-ai.providers.lm-studio.streamIdleTimeoutMs`——由 DSH 的提供方 schema 校验，并对该提供方的下一个请求立即生效，对话与任务执行都受影响。**恢复默认** 会移除覆盖。超时按提供方生效（同一路由上的全部模型共享该值），因为这是 DSH 的原生粒度；看板只是编辑 Models 页面所编辑的同一份设置，不涉及 Host、账本或协议的变更。
...
4. 打开设置卡片（设置 → 插件 → 全部任务）→ **端点**，添加一个端点（如服务于本地提供方的 `lm-studio`），确认新建任务弹窗的「端点」下拉立即列出它。再配置一个 `offPeakOnly: true`（或窄 `allowedHours`）的端点并钉到任务上，在时段外执行：任务显示**等待端点**，不创建会话，时段一开自动启动；同时确认执行记录里显示实际使用的端点。
...
7. 在设置卡片（设置 → 插件 → 全部任务）中打开**模型默认超时**，把一个本地提供方（如 `lm-studio`）的空闲超时提高到 600 秒并保存；确认 `~/.dsh/settings.yaml` 出现 `llm-pi-ai.providers.lm-studio.streamIdleTimeoutMs`，然后用该模型执行任务，确认生成块之间的长空闲间隙不再失败。**恢复默认** 会移除覆盖并回到 300 秒。
