...
dsh Web GUI 的 Host 权威多列任务看板（独立仓库，源自 zhu1090093659/dsh-web 的 dsh-task-board v0.3.6）。任务通过真实 DSH 会话执行，浏览器只负责异步展示与提交动作。

## Host 账本、执行与调度

- v2 权威账本固定为 `$DSH_HOME/all-tasks/ledger-v2.json`，结构是 `{ schemaVersion: 2, revision, tasks, scheduler }`；写入必须保持临时文件加原子 rename、损坏文件隔离和 revision 单调递增。
- 浏览器 `dsh.allTasks.v1` 只用于一次性导入且必须保留；导入 marker 只能在 Host 确认后写。所有生产变更走 `protocol.ts` 的严格同源 action 协议，UI 不得先写未确认状态。
- 手动与 cron 统一走 `HostExecutionRunner`。钉住的 workspace、agent preset、model、permission 任一失效都在任务 Prompt 前 fail closed；每次 execution 默认创建独立会话，但顺序分组的 maintainSession 例外：组内成员复用同一会话，且成员的工作区/预设必须与共享会话一致，否则同样 fail closed。
- 分组的 `finalStepTaskId` 指定一个「最终步骤」成员：它被排除在并行批次与常规推进链之外，直到其他所有成员都已结算（`groupFinalStepReady` 闸门）才会被推进流程启动；手动运行被闸门拒绝。成员被移出分组时该指定随之清除。
...
- AI 会话通过 `ctx.tools` 上的 `task_create` / `task_list` / `task_get` 工具创建与查询任务（`src/task-tools.ts`，看板启用时注册）。`task_create` 只入队不运行：默认 `approved: false`（需人工批准），来源标 `agent`；校验走账本同一条 fail-closed `create` 路径。
...
- Host 功能只依赖官方 `@deepseek-ai/*` NPM SDK，不得导入 DSH 源码。`src/dsh-home.ts`、`src/loopback.ts` 与 `build/` 下的构建预设是从 dsh-web monorepo vendored 的副本，本仓库没有同步脚本，改动需手动带过。
