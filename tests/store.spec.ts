...
describe('schedule persistence', () => {
  ...
  it('round-trips the model pin and repairs malformed selections', () => {
    const base = {
      id: 't1', title: 'a', description: '', prompt: 'a',
      createdAt: NOW, updatedAt: NOW, executions: [], status: 'todo' as const,
    }
    const kept = parseLedger(JSON.stringify([{ ...base, model: { provider: 'deepseek', model: 'deepseek-chat' } }]))
    expect(kept[0].model).toEqual({ provider: 'deepseek', model: 'deepseek-chat' })
    const repaired = parseLedger(JSON.stringify([
      { ...base, model: 'deepseek' },
      { ...base, model: { provider: '', model: 'deepseek-chat' } },
      { ...base, model: { provider: 'deepseek', model: 'deepseek-chat', reasoningEffort: 7 } },
    ]))
    for (const task of repaired) expect(task.model).toBeUndefined()
