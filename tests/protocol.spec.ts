  ...
  it('accepts a group create and normalizes its optional fields', () => {
    const parsed = parseActionEnvelope({
      ...
      action: {
        ...
        input: { name: ' Nightly ', mode: 'parallel', maxParallel: 2, endpoints: [' cloud ', 'local'], allowedHours: { start: '22:00', end: '06:00' }, offPeakOnly: true, maintainSession: true, compactBetween: true, schedule: { enabled: true, cron: '0 2 * * *' } },
      },
    })
