...
describe('use-case: update model pin', () => {
  it('sets, replaces, and clears the model pin', () => {
    const model = { provider: 'deepseek', model: 'deepseek-chat' }
    const pinned = applyUpdateTask(seed(2), 'id-0', { model }, NOW + 10)
    expect(pinned[0].model).toEqual(model)

    const replaced = applyUpdateTask(pinned, 'id-0', { model: { provider: 'pi-ai', model: 'pi-model' } }, NOW + 11)
    expect(replaced[0].model).toEqual({ provider: 'pi-ai', model: 'pi-model' })

    const cleared = applyUpdateTask(replaced, 'id-0', { model: null }, NOW + 12)
    expect(cleared[0].model).toBeUndefined()

    const clearedUndefined = applyUpdateTask(pinned, 'id-0', { model: undefined }, NOW + 13)
    expect(clearedUndefined[0].model).toBeUndefined()
  })

  it('normalizes a malformed model pin away instead of storing it', () => {
    const updated = applyUpdateTask(seed(2), 'id-0', { model: { provider: '', model: 'deepseek-chat' } as never }, NOW + 10)
