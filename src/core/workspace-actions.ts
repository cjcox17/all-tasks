/**
 * Workspace-level run/stop planning: a pure projection of one workspace's (or
 * the whole board's) actionable tasks and groups. The landing list uses it to
 * decide which control icons are enabled; the controller uses it to fan the
 * per-task / per-group actions out. Workspace pausing is the board-level soft
 * pause (`pause-workspace`) handled by the Host ledger, not a fan-out, so this
 * plan carries no pause set. Framework-free and unit-testable, like the other
 * core projections.
 */
import { orderedGroupMembers, type TaskGroupRecord } from './groups.ts'
import type { TaskRecord } from './tasks.ts'

/** The actionable ids for one workspace's run / stop controls. */
...
/**
 * Compute the run/stop plan for one workspace (or the whole board when
 ...
 * group with an in-scope member; Stop cancels the in-scope running ungrouped
 * tasks and running groups.
 */
export function planWorkspaceActions(
  ...
  return { todoTaskIds, runnableGroupIds, stoppableTaskIds, stoppableGroupIds }
}
