...
import {
  ALL_TASKS_API_PREFIX,
  type AllTasksAction,
  type AllTasksActionEnvelope,
  type AllTasksEventPayload,
  type AllTasksSnapshot,
} from '../protocol.ts'

const IMPORT_MARKER = 'dsh.allTasks.v2.hostImported'
const SOURCE_KEY = 'dsh.allTasks.v2.sourceId'
const IMPORT_REQUEST_KEY = 'dsh.allTasks.v2.importRequestId'
...
export interface AllTasksHostTransport {
...
export class HttpAllTasksHostTransport implements AllTasksHostTransport {
  constructor(private readonly storage: Pick<Storage, 'getItem' | 'setItem'> | undefined = globalThis.localStorage) {}

  async bootstrap(legacy: readonly TaskRecord[]): Promise<AllTasksSnapshot> {
  ...
  async state(): Promise<AllTasksSnapshot> {
    return await this.request(`${ALL_TASKS_API_PREFIX}/state`, { cache: 'no-store' })
  }

  async action(action: AllTasksAction): Promise<AllTasksSnapshot> {
    return await this.post(uuid(), action)
  }

  private async post(requestId: string, action: AllTasksAction): Promise<AllTasksSnapshot> {
    const envelope: AllTasksActionEnvelope = { requestId, action }
    return await this.request(`${ALL_TASKS_API_PREFIX}/action`, {
      method: 'POST',
  ...
  private async request(url: string, init: RequestInit): Promise<AllTasksSnapshot> {
    const controller = new AbortController()
    const timeout = globalThis.setTimeout(() => { controller.abort() }, REQUEST_TIMEOUT_MS)
    try {
      return await readJson<AllTasksSnapshot>(await fetch(url, { ...init, signal: controller.signal }))
    } catch (error) {
      if (controller.signal.aborted) throw new Error(`all-tasks Host request timed out after ${REQUEST_TIMEOUT_MS / 1_000}s`)
      throw error
    } finally {
  ...
  subscribe(listener: (event?: AllTasksEventPayload) => void): () => void {
    const events = new EventSource(`${ALL_TASKS_API_PREFIX}/events`)
    events.onmessage = (message: MessageEvent<string>): void => {
      try {
        const parsed = JSON.parse(message.data) as AllTasksEventPayload
