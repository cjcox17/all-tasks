...
export const zh = {
  ...
  'exec.hint': '执行时生效：工作区决定执行会话落在哪个工作区；模式决定会话的 agent 预设；端点按优先级把任务路由到计算后端（第一个可用者胜出，均不可用时排队等待）；模型把执行会话钉到提供商与模型，可再选推理强度；权限经 /permission 命令应用到会话。留空则使用运行时默认。',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'exec.hint': 'Applied when the task runs: the workspace decides where the execution session lands; the mode composes the session\'s agent preset; endpoints route the run through the first eligible compute backend in priority order (waiting when none is eligible); the model pins the session to a specific provider and model, optionally with a reasoning-effort level; the permission is applied through the /permission command. Blank = runtime default.',
