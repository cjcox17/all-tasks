...
export const zh = {
  ...
  'exec.hint': '执行时生效：工作区决定执行会话落在哪个工作区；模式决定会话的 agent 预设；模型把执行会话钉到指定的提供商与模型，可再选推理强度；权限经 /permission 命令应用到会话。留空则使用运行时默认。',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'exec.hint': 'Applied when the task runs: the workspace decides where the execution session lands; the mode composes the session\'s agent preset; the model pins the execution session to a specific provider and model, optionally with a reasoning-effort level; the permission is applied through the /permission command. Blank = runtime default.',
