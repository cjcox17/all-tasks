...
export const zh = {
  ...
  'settings.endpointsHint': '端点是模型路由器调度任务运行的计算目标（DeepSeek Official、NAS 上的 LM Studio 等）。保存后立即生效：任务弹窗的「端点」下拉与路由器都会实时刷新。',
  'settings.endpointNew': '新端点',
  'settings.endpointId': 'ID（任务引用它）',
  'settings.endpointName': '显示名称',
  'settings.endpointProvider': '提供方（llm 提供方路由）',
  'settings.endpointModels': '模型（逗号分隔，留空=全部）',
  'settings.endpointDefaultModel': '默认模型',
  'settings.endpointMaxConcurrency': '最大并发',
  'settings.endpointMaxTokens': 'Token 上限',
  'settings.endpointAllowedHours': '允许时段（HH:MM）',
  'settings.endpointOffPeak': 'Off-peak 窗口',
  'settings.endpointOffPeakOnly': '仅 off-peak 运行',
  ...
  'settings.endpointInvalidProvider': '提供方不能为空。',
  'settings.endpointInvalidNumber': '请输入不小于 1（并发）或 0（token）的整数。',
  'settings.endpointInvalidTime': '时间需为 HH:MM 或成对留空。',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'settings.endpointsHint': 'Endpoints are the compute targets the endpoint model-router schedules task runs through (DeepSeek Official, an LM Studio on the NAS, …). Saving applies live: the task modal\'s Endpoints dropdown and the router pick the change up immediately.',
  ...
  'settings.endpointModels': 'Models (comma-separated; blank = all)',
  'settings.endpointDefaultModel': 'Default model',
  'settings.endpointMaxConcurrency': 'Max concurrency',
  'settings.endpointMaxTokens': 'Token cap',
  'settings.endpointAllowedHours': 'Allowed hours (HH:MM)',
  'settings.endpointOffPeak': 'Off-peak window',
  'settings.endpointOffPeakOnly': 'Off-peak only',
  ...
  'settings.endpointInvalidProvider': 'Provider is required.',
  'settings.endpointInvalidNumber': 'Enter a whole number >= 1 (concurrency) or >= 0 (tokens).',
  'settings.endpointInvalidTime': 'Times must be HH:MM or blank in pairs.',
