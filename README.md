...
Endpoints live under the `task-board` settings namespace. The settings card (**All Tasks → settings → Endpoints**) is the normal editor: it lists every endpoint, adds/removes/reorders them, sets their fields, and writes the whole list on Save — the task modal's Endpoints dropdown and the router reload live, no restart. The same values are stored as YAML under `task-board` and may be edited directly too (the router reloads them live as well):
