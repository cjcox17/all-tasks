...
- **Approval gate**: every task carries an approval state. Tasks created manually are approved by default; programmatic creation (the protocol `create` action or an event webhook) mints a task **unapproved by default** and tags it with an origin badge (**API** / **Webhook**), so a card that appears by itself is immediately identifiable and can never run until approved. An unapproved task can never be run by any means — manual Run/Rerun, its own cron, and group auto-advance are all refused until it is approved again (a group sequence skips it and continues with the next member). Unapproved tasks stay fully manageable: they render with a **Not approved** badge, can be moved/edited/grouped, and the header's **Unapproved only** filter shows exactly what needs approval for the day. Approve with one click from the card (✓), the task detail, or the group member row; the task detail also offers **Unapprove**. A queued run whose task is unapproved while waiting is cancelled (lands in the Failed column), and a task cron held while unapproved rolls forward to its next occurrence instead of firing late.
...
- **Default**: tasks created manually (the new-task dialog) are approved. Programmatic creation through the protocol `create` action (a script, another tool, or an agent) mints the task **unapproved by default** — the intended flow for anything that should wait for a human's sign-off (e.g. tasks created by another tool for the day's queue). A programmatic creator that genuinely wants immediate execution passes `approved: true` explicitly (the event webhook's autoRun flow does); the board dialog always claims the `user` origin.
...
- **Origin**: every task records who created it — the board dialog (`user`), a script/tool through the protocol `create` action (`api`), or the event webhook (`event`). Non-user tasks carry an **API** / **Webhook** badge on the card, the workspace-list rows, and the detail view, so a task that "shows up by itself" while you work is immediately distinguishable from your own — and since those tasks are minted unapproved by default, nothing appears and starts without you approving it first.
