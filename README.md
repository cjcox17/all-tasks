...
- **Workspace overview list**: the board opens on a Monday.com-style board table — a column header (Workspace / To do / Pending / Working / Scheduled / Finished / Failed / Total) above one row per workspace with a color-coded avatar and right-aligned tabular counts, plus an **All tasks** row aggregating every workspace (including unassigned tasks). Clicking a row opens that workspace's kanban; the ▸ chevron expands the row inline into its groups (colored section headers with a name pill, mode badge, member count, and a per-group collapse toggle) and its task rows (status dot, title, aligned status label and badges), and clicking a task row opens the task detail; the ⚙ button opens the workspace default-settings editor. The kanban is always workspace-scoped now, the header's **back** button returns to the overview, and the old workspace dropdown is gone. A scoped view shows that workspace's pinned tasks per column, then an **Unassigned** section for tasks without a pin (they fall back to the recent workspace at run time), so nothing disappears from any view; only that workspace's groups appear in the main columns, and unassigned-scope groups live inside the **Unassigned** section. Workspaces pinned by tasks but missing from the runtime list (deleted or renamed) still appear in the overview, so pinned tasks never become unreachable.
- **Board dashboard & workspace controls**: the landing view opens with a summary strip — tasks (todo / pending / running / queued), completed (with success rate), failed (with total runs), scheduled + groups, token totals, and an estimated cost. Token totals are captured from each session's `assistant/message` usage at settlement (input/output/cache/reasoning), so only runs started after this change carry them; cost multiplies those tokens by the configured USD-per-1M-token prices (Settings → Plugins → All Tasks) and shows "—" until both exist. Workspace rows are expanded by default (foldable per group and per workspace), and each row carries ▶ run / ⏸ pause / ⏹ stop controls: run starts its ungrouped **todo** tasks (never backlog) plus its non-stopped groups, pause stops its running groups (resumable), and stop cancels its running ungrouped tasks and groups.
...
- Same-origin endpoints are `GET /api/all-tasks/state`, `GET /api/all-tasks/events`, `POST /api/all-tasks/action`, and `POST /api/all-tasks/title-suggest` (the title-generation RPC; advisory only — it never touches the ledger).
...
| `autoTitle` | `true` | When the new-task title is blank, generates one from the run prompt through a short backend session (the prompt's first line is the fallback; disabled means no generation, the prompt-line fallback still applies at submit). |
...
- **Joining a running group**: a task that joins a group whose sequence has already started — created into it, edited into it, or dragged onto it — is marked **Held** (a small badge with a tooltip on the card). The auto-advance chain skips held members, so the new task never starts on its own; it waits in its column until you start it explicitly. The member's **Run** button, the group's **Start group** button, and the next group-cron fire are all explicit starts that clear the hold and launch it. Joining a fresh, never-run group behaves as before (no hold). A held member stays fully manually startable either way.
...
- **Move the whole group**: grab anywhere on the group banner and drag it onto the Backlog or To Do column to move every member at once (refused while any member is running or the group is stopped) instead of dragging cards one by one. A floating ghost follows the pointer during every drag, with an insertion line showing where a drop will land.
